...
function upload_files() {
    ...
    if ($_FILES['file']['name'] !== "") {
        ...
        if( ! in_array($extension, $extensiones)) {
            $error .=  'Only allowed upload files with these extensions: ' . implode(', ', $extensiones).' <br>';
        }
        ////////////////////////////////////////////////////////////////////////////
    ...
    if ($error == "") {
        if ($copiarFichero) {
            ...
            $upfile ='/proyecto_v3/media/'.$nombreFichero;
...
function remove_files(){
	...
	if(file_exists($_SERVER['DOCUMENT_ROOT'].'/proyecto_v3/media/'.$_SESSION['nombreFichero'])){
		unlink($_SERVER['DOCUMENT_ROOT'].'/proyecto_v3/media/'.$_SESSION['nombreFichero']);
		return true;
	}else{
		return false;
	}
	//return "remove files okkkkkk";
